                            ...
                            GlobalVariable *&lib, GlobalVariable *&sym)
{
    ...
    if (llvmgv == NULL) {
        ...
        name += "_";
        name += std::to_string(globalUniqueGeneratedNames++);
        auto T_pvoidfunc = JuliaType::get_pvoidfunc_ty(getLLVMContext(M));
        ...
        GlobalVariable *llvmgv, bool runtime_lib)
{
    ...
    //   }
    //   return (*llvmgv)
    auto T_pvoidfunc = JuliaType::get_pvoidfunc_ty(getLLVMContext(irbuilder));
    BasicBlock *enter_bb = irbuilder.GetInsertBlock();
    BasicBlock *dlsym_lookup = BasicBlock::Create(getLLVMContext(irbuilder), "dlsym");
    BasicBlock *ccall_bb = BasicBlock::Create(getLLVMContext(irbuilder), "ccall");
    ...
    if (lib_expr) {
        ...
        if (runtime_lib) {
            ...
            libname = ConstantExpr::getIntToPtr(ConstantInt::get(getSizeTy(getLLVMContext(irbuilder)), (uintptr_t)f_lib), getInt8PtrTy(irbuilder));
        }
        ...
        bool runtime_lib)
{
    ...
    GlobalVariable *got = new GlobalVariable(*M, T_pvoidfunc, false,
                                             ...
                                             fname);
    BasicBlock *b0 = BasicBlock::Create(getLLVMContext(M), "top", plt);
...
std::string generate_func_sig(const char *fname)
{
    ...
    sret = 0;
    LLVMContext &LLVMCtx = getLLVMContext(lrt);
