...
LLVMContext &getLLVMContext(LLVM_T &llvmt) {
...
LLVMContext &getLLVMContext(LLVMContext &ctx) {
    return ctx;
}

class jl_codectx_t;
LLVMContext &getLLVMContext(jl_codectx_t &ctx);
...
struct jl_tbaacache_t {
    ...
    void initialize(llvm::LLVMContext &context) {
        if (initialized) {
            assert(&getLLVMContext(tbaa_root) == &context);
            return;
        }
        initialized = true;
...
public:
    ...
    GlobalVariable *realize(Module *m) {
        ...
        return new GlobalVariable(*m, _type(getLLVMContext(m)),
...
public:
    ...
    llvm::Function *realize(llvm::Module *m) {
        ...
        llvm::Function *F = llvm::Function::Create(_type(getLLVMContext(m)),
                         llvm::Function::ExternalLinkage,
                         name, m);
        if (_attrs)
            F->setAttributes(_attrs(getLLVMContext(m)));
        return F;
    }
};
...
struct jl_cgval_t {
    ...
    {
        if (Vboxed)
            assert(Vboxed->getType() == JuliaType::get_prjlvalue_ty(getLLVMContext(Vboxed)));
    ...
    explicit jl_cgval_t(jl_value_t *typ) : // ghost value constructor
        // mark explicit to avoid being used implicitly for conversion from NULL (use jl_cgval_t(getLLVMContext(ctx)) instead)
        V(NULL),
        Vboxed(NULL),
        TIndex(NULL),
...
public:
    ...
    jl_typecache_t &types() {
        type_cache.initialize(getLLVMContext(builder));
        return type_cache;
    }

    jl_tbaacache_t &tbaa() {
        tbaa_cache.initialize(getLLVMContext(builder));
        return tbaa_cache;
    }
...
static void undef_derived_strct(IRBuilder<> &irbuilder, Value *ptr, jl_datatype_t *sty, MDNode *tbaa)
{
    ...
    if (np == 0)
        return;
    auto T_prjlvalue = JuliaType::get_prjlvalue_ty(getLLVMContext(irbuilder));
...
static inline jl_cgval_t ghostValue(jl_codectx_t &ctx, jl_value_t *typ)
{
    if (typ == jl_bottom_type)
        return jl_cgval_t(getLLVMContext(ctx)); // Undef{}
...
static inline jl_cgval_t update_julia_type(jl_codectx_t &ctx, const jl_cgval_t &v, jl_value_t *typ)
{
    ...
    if (jl_is_concrete_type(v.typ) && !jl_is_kind(v.typ)) {
        if (jl_is_concrete_type(typ) && !jl_is_kind(typ)) {
            ...
            return jl_cgval_t(getLLVMContext(ctx));
        }
...
static void CreateTrap(IRBuilder<> &irbuilder, bool create_new_block)
{
    ...
    if (create_new_block) {
        BasicBlock *newBB = BasicBlock::Create(getLLVMContext(irbuilder), "after_noret", f);
...
{
    Function *f = irbuilder.GetInsertBlock()->getParent();
    BasicBlock *abortBB = BasicBlock::Create(getLLVMContext(irbuilder), "debug_abort", f);
    BasicBlock *postBB = BasicBlock::Create(getLLVMContext(irbuilder), "post_abort", f);
...
#if JL_LLVM_VERSION >= 140000
    AttrBuilder attr(getLLVMContext(F));
#else
    AttrBuilder attr;
#endif
                            ...
                            const jl_cgval_t *argv, size_t nargs, const jl_cgval_t *modifyop)
{
    ...
    if (order == jl_memory_order_invalid || order == jl_memory_order_notatomic) {
        ...
        *ret = jl_cgval_t(getLLVMContext(ctx)); // unreachable
        return true;
    }
    ...
    if (jl_is_datatype(uty) && jl_struct_try_layout(uty)) {
        ...
        if (idx != -1) {
            ...
            if (!jl_has_free_typevars(ft)) {
                ...
                *ret = jl_cgval_t(getLLVMContext(ctx));
                ...
                if (!isboxed && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
                    ...
                    if (!isboxed && jl_is_uniontype(ety)) {
                        ...
                        typed_store(ctx,
                                    ...
                                    idx, val, jl_cgval_t(getLLVMContext(ctx)), ety,
                                    isboxed ? ctx.tbaa().tbaa_ptrarraybuf : ctx.tbaa().tbaa_arraybuf,
                                    ctx.aliasscope,
                                    data_owner,
                                    isboxed,
...
static jl_cgval_t emit_call(jl_codectx_t &ctx, jl_expr_t *ex, jl_value_t *rt)
{
    ...
    for (size_t i = 1; i < nargs; ++i) {
        ...
        if (argv[i].typ == jl_bottom_type)
            return jl_cgval_t(getLLVMContext(ctx)); // anything past here is unreachable
    }
...
static jl_cgval_t emit_expr(jl_codectx_t &ctx, jl_value_t *expr, ssize_t ssaval)
{
    ...
    if (jl_is_ssavalue(expr)) {
        ...
        if (!ctx.ssavalue_assigned.at(idx)) {
            ...
            return jl_cgval_t(getLLVMContext(ctx)); // dead code branch
        }
        else {
    ...
    if (head == jl_isdefined_sym) {
        ...
        jl_cgval_t argv[2] = {jl_cgval_t(getLLVMContext(ctx)), jl_cgval_t(getLLVMContext(ctx))};
        assert(nargs == 2);
        ...
        SmallVector<jl_cgval_t, 4> argv(nargs, jl_cgval_t(getLLVMContext(ctx)));
        for (size_t i = 0; i < nargs; ++i) {
            argv[i] = emit_expr(ctx, args[i]);
        }
...
{
    ++EmittedToJLInvokes;
    jl_codectx_t ctx(getLLVMContext(M), params);
    std::string name;
...
const char *jl_generate_ccallable(LLVMOrcThreadSafeModuleRef llvmmod, void *sysimg_handle, jl_value_t *declrt, jl_value_t *sigt, jl_codegen_params_t &params)
{
    ...
    if (toboxed)
        lcrt = JuliaType::get_prjlvalue_ty(getLLVMContext(lcrt));
        ...
        Module *M, jl_codegen_params_t &params)
{
    ...
    Function *w = Function::Create(JuliaType::get_jlfunc_ty(getLLVMContext(M)), GlobalVariable::ExternalLinkage, funcName, M);
    addRetAttr(w, Attribute::NonNull);
    w->addFnAttr(Attribute::get(getLLVMContext(M), "thunk"));
    jl_init_function(w);
        ...
        jl_codegen_params_t &params)
{
    ...
    ctx.SAvalues.assign(n_ssavalues, jl_cgval_t(getLLVMContext(ctx)));
    ...
    for (auto &tup : ctx.PhiNodes) {
        ...
        for (size_t i = 0; i < jl_array_len(edges); ++i) {
            ...
            if (!terminator->getParent()->getUniqueSuccessor()) {
                ...
                BasicBlock *NewBB = BasicBlock::Create(getLLVMContext(ctx),
