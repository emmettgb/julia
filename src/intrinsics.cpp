...
static Constant *julia_const_to_llvm(jl_codectx_t &ctx, const void *ptr, jl_datatype_t *bt)
{
    ...
    if (lt->isFloatTy()) {
        ...
        return ConstantFP::get(getContext(ctx),
    ...
    if (lt->isFloatingPointTy() || lt->isIntegerTy() || lt->isPointerTy()) {
        ...
        if (lt->isPointerTy()) {
            Type *Ty = IntegerType::get(getContext(ctx), 8 * nb);
    ...
    for (size_t i = 0; i < nf; i++) {
        ...
        unsigned llvm_idx = isa<StructType>(lt) ? convert_struct_offset(getModule(ctx)->getDataLayout(), lt, offs) : i;
        ...
        if (jl_is_uniontype(ft)) {
            ...
            Type *AlignmentType = IntegerType::get(getContext(ctx), 8 * al);
...
static Value *emit_unbox(jl_codectx_t &ctx, Type *to, const jl_cgval_t &x, jl_value_t *jt, Value *dest, MDNode *tbaa_dest, bool isVolatile)
{
    ...
    if (jt == (jl_value_t*)jl_bool_type || to == getInt1Ty(ctx)) {
        ...
        if (jt == (jl_value_t*)jl_bool_type)
            unbox_load->setMetadata(LLVMContext::MD_range, MDNode::get(getContext(ctx), {
...
static jl_cgval_t generic_bitcast(jl_codectx_t &ctx, const jl_cgval_t *argv)
{
    ...
    Type *llvmt = bitstype_to_llvm(bt, getContext(ctx));
    int nb = jl_datatype_size(bt);
        ...
        const jl_cgval_t *argv, bool toint, bool fromint)
{
    ...
    Type *to = bitstype_to_llvm(jlto, getContext(ctx));
    Type *vt = bitstype_to_llvm(v.typ, getContext(ctx));
    if (toint)
        to = INTT(to);
    else
        to = FLOATT(to);
...
static jl_cgval_t emit_pointerref(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    Value *idx = emit_unbox(ctx, getSizeTy(getContext(ctx)), i, (jl_value_t*)jl_long_type);
    Value *im1 = ctx.builder.CreateSub(idx, ConstantInt::get(getSizeTy(getContext(ctx)), 1));
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        im1 = ctx.builder.CreateMul(im1, ConstantInt::get(getSizeTy(getContext(ctx)),
...
static jl_cgval_t emit_pointerset(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        thePtr = emit_unbox(ctx, getSizePtrTy(ctx), e, e.typ);
        Instruction *store = ctx.builder.CreateAlignedStore(
          ctx.builder.CreatePtrToInt(emit_pointer_from_objref(ctx, boxed(ctx, x)), getSizeTy(getContext(ctx))),
            ctx.builder.CreateInBoundsGEP(getSizeTy(getContext(ctx)), thePtr, im1), Align(align_nb));
        ...
        if (!type_is_ghost(ptrty)) {
            ...
            typed_store(ctx, thePtr, im1, x, jl_cgval_t(getContext(ctx)), ety, ctx.tbaa().tbaa_data, nullptr, nullptr, isboxed,
...
static jl_cgval_t emit_atomic_pointerref(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (!jl_isbits(ety)) {
        ...
        Value *strct = emit_allocobj(ctx, size,
                                     literal_pointer_val(ctx, ety));
        Value *thePtr = emit_unbox(ctx, getInt8PtrTy(ctx), e, e.typ);
        Type *loadT = Type::getIntNTy(getContext(ctx), nb * 8);
...
static Value *emit_checked_srem_int(jl_codectx_t &ctx, Value *x, Value *den)
{
    ...
    BasicBlock *m1BB = BasicBlock::Create(getContext(ctx), "minus1", ctx.f);
    BasicBlock *okBB = BasicBlock::Create(getContext(ctx), "oksrem", ctx.f);
    BasicBlock *cont = BasicBlock::Create(getContext(ctx), "after_srem", ctx.f);
...
static jl_cgval_t emit_ifelse(jl_codectx_t &ctx, jl_cgval_t c, jl_cgval_t x, jl_cgval_t y, jl_value_t *rt_hint)
{
    ...
    if (t1 == jl_bottom_type && t2 == jl_bottom_type)
        return jl_cgval_t(getContext(ctx)); // undefined
        ...
        if (x_tindex || y_tindex) {
            ...
            if (!x_ptr && !y_ptr) { // both ghost
                ...
                if (ifelse_tbaa == NULL) {
                    ...
                    ifelse_tbaa = MDNode::get(getContext(ctx), Ops);
                }
            }
            Value *tindex;
            ...
            if (x_tindex && y_tindex) {
                ...
                BasicBlock *post = BasicBlock::Create(getContext(ctx), "post", ctx.f);
                BasicBlock *compute = BasicBlock::Create(getContext(ctx), "compute_tindex", ctx.f);
...
static jl_cgval_t emit_intrinsic(jl_codectx_t &ctx, intrinsic f, jl_value_t **args, size_t nargs)
{
    ...
    case not_int: {
        ...
        if (!jl_is_primitivetype(x.typ))
            return emit_runtime_call(ctx, f, argv, nargs);
        Type *xt = INTT(bitstype_to_llvm(x.typ, getContext(ctx)));
    ...
    case have_fma: {
        ...
        FunctionCallee intr = getModule(ctx)->getOrInsertFunction(intr_name, getInt1Ty(ctx));
    ...
    default: {
        ...
        if (!jl_is_primitivetype(xinfo.typ))
            return emit_runtime_call(ctx, f, argv, nargs);
        Type *xtyp = bitstype_to_llvm(xinfo.typ, getContext(ctx));
        if (float_func()[f])
            xtyp = FLOATT(xtyp);
        else
            xtyp = INTT(xtyp);
        ...
        if (f == shl_int || f == lshr_int || f == ashr_int) {
            if (!jl_is_primitivetype(argv[1].typ))
                return emit_runtime_call(ctx, f, argv, nargs);
            argt[1] = INTT(bitstype_to_llvm(argv[1].typ, getContext(ctx)));
        }
        else {
                                     ...
                                     jl_datatype_t **newtyp, jl_value_t *xtyp)
{
    ...
    case fma_float: {
        ...
        FunctionCallee fmaintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::fma, makeArrayRef(t));
    ...
    case checked_umul_int: {
        ...
        FunctionCallee intr = Intrinsic::getDeclaration(getModule(ctx), intr_id, makeArrayRef(t));
    ...
    case bswap_int: {
        FunctionCallee bswapintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::bswap, makeArrayRef(t));
        return ctx.builder.CreateCall(bswapintr, x);
    }
    case ctpop_int: {
        FunctionCallee ctpopintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::ctpop, makeArrayRef(t));
        return ctx.builder.CreateCall(ctpopintr, x);
    }
    case ctlz_int: {
        FunctionCallee ctlz = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::ctlz, makeArrayRef(t));
        y = ConstantInt::get(getInt1Ty(ctx), 0);
        return ctx.builder.CreateCall(ctlz, {x, y});
    }
    case cttz_int: {
        FunctionCallee cttz = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::cttz, makeArrayRef(t));
        y = ConstantInt::get(getInt1Ty(ctx), 0);
        return ctx.builder.CreateCall(cttz, {x, y});
    }

    case abs_float: {
        FunctionCallee absintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::fabs, makeArrayRef(t));
        return ctx.builder.CreateCall(absintr, x);
    }
    case copysign_float: {
        FunctionCallee copyintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::copysign, makeArrayRef(t));
    ...
    case ceil_llvm: {
        FunctionCallee ceilintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::ceil, makeArrayRef(t));
        return ctx.builder.CreateCall(ceilintr, x);
    }
    case floor_llvm: {
        FunctionCallee floorintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::floor, makeArrayRef(t));
        return ctx.builder.CreateCall(floorintr, x);
    }
    case trunc_llvm: {
        FunctionCallee truncintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::trunc, makeArrayRef(t));
        return ctx.builder.CreateCall(truncintr, x);
    }
    case rint_llvm: {
        FunctionCallee rintintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::rint, makeArrayRef(t));
        return ctx.builder.CreateCall(rintintr, x);
    }
    case sqrt_llvm: {
        FunctionCallee sqrtintr = Intrinsic::getDeclaration(getModule(ctx), Intrinsic::sqrt, makeArrayRef(t));
