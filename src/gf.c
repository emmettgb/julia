                                     ...
                                     jl_code_instance_t *ci JL_ROOTED_ARGUMENT JL_MAYBE_UNROOTED)
{
    ...
    jl_value_t *oldci = jl_atomic_load_relaxed(&mi->cache);
        ...
        jl_svec_t *sparams)
{
    ...
    if (mt && cachett == tt && jl_svec_len(guardsigs) == 0 && tt->hash && !tt->hasfreetypevars) {
        ...
        jl_value_t *oldcache = jl_atomic_load_relaxed(&mt->leafcache);
        ...
        jl_value_t *newcache = jl_eqtable_put(jl_atomic_load_relaxed(&mt->leafcache), (jl_value_t*)tt, (jl_value_t*)newentry, NULL);
...
jl_method_instance_t *jl_get_unspecialized(jl_method_t *def JL_PROPAGATES_ROOT)
{
    ...
    jl_value_t *unspec = jl_atomic_load_relaxed(&def->unspecialized);
    if (unspec == NULL) {
