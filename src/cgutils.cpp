        ...
        const std::string &txt)
{
    ...
    Constant *Data = ConstantDataArray::get(getLLVMContext(irbuilder), arrayRefFromStringRef(ctxt));
...
static Value *julia_pgv(jl_codectx_t &ctx, const char *cname, void *addr)
{
    ...
    gv->setMetadata("julia.constgv", MDNode::get(getLLVMContext(gv), None));
...
static inline void maybe_mark_argument_dereferenceable(Argument *A, jl_value_t *jt)
{
    ...
    AttrBuilder B(getLLVMContext(A));
#else
    AttrBuilder B;
#endif
                                                           ...
                                                           size_t size, size_t align)
{
    if (isa<PointerType>(LI->getType())) {
        if (!can_be_null)
            // The `dereferencable` below does not imply `nonnull` for non addrspace(0) pointers.
            LI->setMetadata(LLVMContext::MD_nonnull, MDNode::get(getLLVMContext(LI), None));
        if (size) {
            Metadata *OP = ConstantAsMetadata::get(ConstantInt::get(getInt64Ty(getLLVMContext(LI)), size));
            LI->setMetadata(can_be_null ? LLVMContext::MD_dereferenceable_or_null : LLVMContext::MD_dereferenceable,
                            MDNode::get(getLLVMContext(LI), { OP }));
            if (align >= 1) {
                Metadata *OP = ConstantAsMetadata::get(ConstantInt::get(getInt64Ty(getLLVMContext(LI)), align));
                LI->setMetadata(LLVMContext::MD_align, MDNode::get(getLLVMContext(LI), { OP }));
            }
        }
    }
    return LI;
...
static Type *_julia_struct_to_llvm(jl_codegen_params_t *ctx, LLVMContext &ctxt, jl_value_t *jt, bool *isboxed, bool llvmcall)
{
    ...
    //  // and always end with an Int8 (selector byte)
    //  lty = ArrayType::get(IntegerType::get(getLLVMContext(lty), 8 * al), fsz / al);
    ...
    //  lty = StructType::get(getLLVMContext(lty), makeArrayRef(Elements));
    // }
    if (isboxed) *isboxed = true;
    return JuliaType::get_prjlvalue_ty(ctxt);
}
...
static jl_cgval_t emit_new_struct(jl_codectx_t &ctx, jl_value_t *ty, size_t nargs, const jl_cgval_t *argv)
{
    ...
    if (nf > 0 || sty->name->mutabl) {
        ...
        for (size_t i = 0; i < nargs; i++) {
            ...
            emit_setfield(ctx, sty, strctinfo, i, rhs, jl_cgval_t(getLLVMContext(ctx)), need_wb, AtomicOrdering::NotAtomic, AtomicOrdering::NotAtomic, false, true, false, false, false, nullptr, "");
        }
        return strctinfo;
    }
    else {
